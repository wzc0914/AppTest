...
namespace Trip.Tests
{
    public abstract class TripTestBase : AbpIntegratedTestBase<TripTestModule>
    {
        ...
        protected void UsingDbContext(Action<TripDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<TripDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<TripDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<TripDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<TripDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TripDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<TripDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TripDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<TripDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<TripDbContext, Task<T>> func)
        {
            T result;
